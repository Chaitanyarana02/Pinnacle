...
import {
  ...
  updateCustomization,
} from "../../store/feature/customization-options.slice";
...
const CustomizationOptionsComponent = () => {
  ...
  const [deleteCustomizationDialog, setDeleteCustomizationDialog] =
    useState<boolean>(false);
  const [addEditDialog, setAddEditDialog] = useState<boolean>(false);
  const [customization, setCustomization] =
    useState<CustomizationProduct>(emptyCustomization);
    ...
    return (
      <>
        <Button
          ...
          outlined
          className="mr-2"
          onClick={() => editCustomization(rowData)}
        />
        <Button
          icon="pi pi-trash"
          rounded
          outlined
          severity="warning"
          onClick={() => confirmDeleteCustomization(rowData)}
        />
      </>
    );
  };
  ...
  const saveCustomization = () => {
    if (customization.optionName.trim()) {
      if (customization.id) {
        dispatch(updateCustomization(customization))
        toast.current?.show({
          severity: "success",
          summary: "Successful",
    ...
    <>
      {customizationState.isLoading ? null : (
        <>
          ...
          <Dialog
            visible={deleteCustomizationDialog}
            style={{ width: "450px" }}
            header="Confirm"
            modal
