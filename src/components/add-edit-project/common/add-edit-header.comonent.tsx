...
const AddEditHeaderComponent = ({
  ...
  const getStep = (number: number) => {
    return (
      <div
        className={`border-circle border-1 w-3rem h-3rem flex align-items-center justify-content-center font-bold text-500  ${
          currentStep > number
            ? "text-white bg-green-500 border-white"
            : "surface-border"
        } ${currentStep === number ? " surface-300 text-primary" : ""}`}
      >
  ...
  return (
    <>
      <div className="flex justify-content-between shadow-1 p-4 align-items-center">
        ...
        <div>
          <i
            className="pi pi-times text-4xl text-500 cursor-pointer"
            onClick={() => {
